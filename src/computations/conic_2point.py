...
def conic_2point(r_1,v_1,t_1,r_2,v_2,t_2,ta_step,mu,rev_check,time_flag) -> float:
	...
	energy=la.norm(v_1,axis=1)**2/2.-mu/la.norm(r_1,axis=1)#orbital energy
	...
	ta_2=get_true_anomaly(p,ecc,r_2,v_2)#true anomaly for point 2
	...
	if len(wrap_ind)>0:
		ta[wrap_ind]=ta_2[wrap_ind]-ta_1[wrap_ind]+2.*np.pi#perform rotation wrapping
	...
	ta_set=np.linspace(np.zeros(np.shape(ta)),ta\
		+2.*np.pi*revs,ta_step,axis=1)+ta_1.T[None].T#true anomaly campling

	pos_set_E=p.T[None].T/(1.+ecc.T[None].T*np.cos(ta_set))*np.cos(ta_set)#E direction position
	pos_set_P=p.T[None].T/(1.+ecc.T[None].T*np.cos(ta_set))*np.sin(ta_set)#P direction position
	pos_set=(np.transpose(E,axes=[2,0,1])*np.transpose(pos_set_E,axes=[2,0,1]).T\
		+np.transpose(P,axes=[2,0,1])*np.transpose(pos_set_P,axes=[2,0,1]).T).T
	...
	vel_set=(np.transpose(E,axes=[2,0,1])*np.transpose(vel_set_E,axes=[2,0,1]).T\
		+np.transpose(P,axes=[2,0,1])*np.transpose(vel_set_P,axes=[2,0,1]).T).T

	#generate times
	if time_flag==1:
		dta_set=np.transpose(ta_set,axes=[2,0,1]).T[1:].T[0]\
			-np.transpose(ta_set,axes=[2,0,1]).T[0]#true anomaly differences
		time_set=np.zeros((np.shape(np.transpose(ta_set,axes=[2,0,1]))))
		time_set[0].T[0]=t_1.T#initial time setting
		tof=find_tof(np.transpose(pos_set,axes=[2,0,1])[0][None],\
			np.transpose(pos_set,axes=[2,0,1])[1:],p,dta_set,mu)/86400.#calculate time of flight
		rev_count=np.floor(dta_set/(2.*np.pi))#count orbits
		revs_adjust=np.where((tof<0))#adjust for TOF algorithm angle wrapping
		rev_count[revs_adjust]=rev_count[revs_adjust]+1
		time_set[0].T[1:]=(t_1+tof+rev_count*period/86400.).T#generate time array
...
def get_true_anomaly(p,ecc,r,v) -> float:
	...
	ta: true anomaly [rad]
	"""
	ta=np.arccos((p/la.norm(r,axis=1)-1.)/ecc)#true anomaly
	quad_check=np.diag(np.dot(np.transpose(r,axes=[2,0,1]),v)[0].T[0]) 
	quad_ind=np.where(quad_check<0)

	if len(quad_ind)>0:
		ta[quad_ind]=2.*np.pi-ta[quad_ind]#apply quadrant fix
	return(ta)
	pass
...
def kep2cart_array(raan,aop,inc) -> float:
	...
	"""
	E=np.array([np.cos(raan)*np.cos(aop)-np.sin(raan)*np.sin(aop)*np.cos(inc),\
		np.sin(raan)*np.cos(aop)+np.cos(raan)*np.sin(aop)*np.cos(inc),\
		np.sin(aop)*np.sin(inc)]).T[0].T[None].T#eccentricity vector
	P=np.array([-np.cos(raan)*np.sin(aop)-np.sin(raan)*np.cos(aop)*np.cos(inc),\
		-np.sin(raan)*np.sin(aop)+np.cos(raan)*np.cos(aop)*np.cos(inc),\
		np.cos(aop)*np.sin(inc)]).T[0].T[None].T#semilatus rectum vector
	H=np.array([np.sin(raan)*np.sin(inc),-np.cos(raan)*np.sin(inc),np.cos(inc)])\
		.T[0].T[None].T#angular momentum vector
...
def find_tof(r0,r,p,delta_ta,mu):
	...
	k=(la.norm(r0,axis=1)*la.norm(r,axis=1)).T*(1.-cos_delta_ta)
	l=(la.norm(r0,axis=1)+la.norm(r,axis=1)).T
	m=(la.norm(r0,axis=1)*la.norm(r,axis=1)).T*(1.+cos_delta_ta)
	...
	if len(ellipse_check)>0:
		tan_half_delta_ta=sin_delta_ta[ellipse_check]\
			/(1.+cos_delta_ta[ellipse_check])
		f_dot=np.sqrt(mu/p[ellipse_check])\
			*tan_half_delta_ta*((1.-cos_delta_ta[ellipse_check])\
			/p[ellipse_check]-1.\
			/la.norm(r0.T[ellipse_check],axis=1)\
			-1./la.norm(r.T[ellipse_check],axis=1))
		sin_delta_E=-la.norm(r0.T[ellipse_check],axis=1)\
			*la.norm(r.T[ellipse_check],axis=1)\
			*f_dot/np.sqrt(mu*a[ellipse_check])#sin of change in eccentric anomaly
		cos_delta_E=1.-la.norm(r0.T[np.unique(ellipse_check)],axis=1)\
			*(1.-f[ellipse_check])\
			/a[ellipse_check]#cosine of change in eccentric anomaly
		...
		TOF[ellipse_check]=g[ellipse_check]+np.sqrt(a[ellipse_check]**3/mu)\
			*(delta_E-sin_delta_E)#elliptical time of flight

	#parabola case
	if len(parabola_check)>0:
		c=np.sqrt(la.norm(r0.T[parabola_check],axis=1)**2\
			+la.norm(r.T[parabola_check],axis=1)**2\
			-2.*la.norm(r0.T[parabola_check],axis=1)\
			*la.norm(r.T[parabola_check],axis=1)\
			*cos_delta_ta[parabola_check])
		s=(la.norm(r0.T[parabola_check],axis=1)\
			+la.norm(r.T[parabola_check],axis=1)+c)/2.
		TOF[parabola_check]=2./3.*np.sqrt(s**3./(2.*mu))*(1.-((s-c)/s)**(3./2.))#parabolic time of flight

	#hyperbola case
	if len(hyperbola_check)>0:
		delta_H=np.arccosh(1+(f[hyperbola_check]-1)\
			*la.norm(r0.T[hyperbola_check],axis=1)\
			/a[hyperbola_check])#change in hyperbolic anomaly
		TOF[hyperbola_check]=g[hyperbola_check]\
			+np.sqrt((-a[hyperbola_check])**3/mu)\
			*(np.sinh(delta_H)-delta_H)#hyperbolic time of flight
	return(TOF)
	pass